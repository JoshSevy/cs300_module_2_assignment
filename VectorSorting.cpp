...
void selectionSort(vector<Bid>& bids) {
    ...
    // pos represents the boundary between the sorted and unsorted portions
    // everything to the left of pos is sorted
    // everything from pos onward is unsorted
    for (size_t pos = 0; pos < bidSize - 1; ++pos) {
        // assume the current position contains the smallest title
        min = pos;
        // iterated over bids and scan the remaining unsorted bids to find the true minimum
        for (size_t i = pos + 1; i < bidSize; ++i) {
            // compare bid titles alphabetically
            // if a smaller title is found, update the minimum index
            if (bids.at(i).title < bids.at(min).title) {
                min = i;
            }
        }
        // if a smaller element was found, swap it with the current position
        // this places the correct element into the sorted portion
        if (min != pos) {
